...
1. **Prepare GPU Environment**: Complete the [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda), and prepare sufficient persistent storage for model caching.
...
Ollama relies on GPU for inference. Please first complete the [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda), and confirm the following:
...
- **Image Selection**: Refer to [Inference Images](./image) and use specific versions (tag/digest) for easier upgrades and rollbacks.
...
- **Upgrade and Rollback**: Perform changes through version management of [Inference Images](./image); it is recommended to operate during off-peak hours.
...
- Check whether the [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda) has been completed.
