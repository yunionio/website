...
1. **Prepare GPU Environment**: Complete the [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda), and plan GPU VRAM, memory, and persistent storage space in advance.
2. **Configure Template Specifications and Image**: Go to [Inference Templates](./template) to configure GPU, CPU, memory, data disk, and [Inference Image](./image) for the vLLM template; the platform usually provides default templates, but you can also create or edit them as needed.
...
vLLM relies on GPU for inference. Please first complete the [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda), and confirm the following:
...
4. Select the corresponding [Inference Image](./image) in the image field, and save the template.
...
- **Image Selection**: Refer to [Inference Images](./image), use specific versions (tag/digest) for easier upgrades and rollbacks, and note CUDA, driver, and vLLM version compatibility.
...
- **Upgrade and Rollback**: Control the runtime version through [Inference Images](./image); evaluate CUDA, driver, image, and model version compatibility before making changes.
...
- Check the driver, CUDA version, and node status according to [NVIDIA and CUDA Environment Setup](/docs/aicloud/getting-started/setup-nvidia-cuda).
