...
1. **Prepare Image, Model, and GPU Environment**: Confirm the target type, [Inference Image](./image), and GPU environment are ready; this document uses `Ollama` templates as an example. If you want the template to pre-mount models, prepare the target models in the [Inference Model Library](./model-library) in advance.
...
2. An [Inference Image](./image) matching the template type has been prepared.
...
- Check whether the template type matches the [Inference Image](./image) type.
