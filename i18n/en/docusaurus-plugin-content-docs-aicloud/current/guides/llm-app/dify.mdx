...
3. **Install model plugins and configure model providers**: To connect to OpenAI, OpenAI-compatible services, or platform-internal inference services such as vLLM, first install the corresponding plugin in the Dify web backend under **Plugins → Marketplace**, then configure the model provider and downstream inference service.
...
- Connecting to platform-internal [inference deployments](../llm-inference/deployment) (e.g. vLLM / SGLang).

Recommended workflow:

1. Log in to the Dify console.
2. Go to **Plugins → Marketplace**.
3. Search for and install the corresponding plugin in the Marketplace, such as `OpenAI`, `OpenAI-API-compatible`, or `vLLM`.
...
- **Connecting to vLLM**: You can use the `vLLM` plugin, or connect via `OpenAI` / `OpenAI-API-compatible` to its OpenAI-compatible API. In most cases, the `Base URL` should point to the `/v1` root path of the service; refer to the field description on the current Dify page for specifics.
...
:::tip
If the downstream is a GPU inference service, the node where the inference service runs still needs to have the GPU environment prepared according to [Configure NVIDIA and CUDA Environment](/docs/aicloud/getting-started/setup-nvidia-cuda).
:::
...
- **Does not directly mount models**: Dify does not support directly mounting model directories to provide model capabilities like vLLM does.
...
- **OpenAI / OpenAI-compatible services**: You typically also need to first install the `OpenAI` or `OpenAI-API-compatible` plugin in Dify's **Plugins → Marketplace**, then fill in the official or compatible service address; when connecting to vLLM via the OpenAI-compatible interface, the common format is `<service-url>/v1`.
- **vLLM integration**: If connecting to platform-internal inference deployments, first verify their availability, then install the corresponding plugin in Dify's **Plugins → Marketplace**, and finally return to the model provider page to complete the integration.
...
- **Upgrades and rollbacks**: Manage runtime versions through [Inference Images](../llm-inference/image). Before upgrading, it is recommended to back up critical data directories and databases.
...
- If the downstream is a platform-internal vLLM service, first verify that the corresponding inference deployment itself is available.
