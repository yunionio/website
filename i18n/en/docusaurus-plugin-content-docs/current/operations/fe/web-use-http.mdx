...
The following steps only apply to platforms deployed using ocboot. Need to set traefik ingress controller to turn off the http redirect to https configuration.

<Tabs>
  <TabItem value="K3s" label="K3s">

K3s uses Traefik v2 as the default ingress controller, which is configured differently from K8s.

Edit the traefik DaemonSet configuration file on the first control node:

```bash
$ vim /var/lib/rancher/k3s/server/manifests/traefik-ds.yaml
```

Find the container args section and comment out the `--entrypoints.web.http.redirections.entrypoint.to=websecure` line:
...
After saving, K3s will automatically detect the changes in the manifests directory and redeploy traefik. No manual restart is needed.
