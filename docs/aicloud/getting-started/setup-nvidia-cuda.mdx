...
- 如果待部署环境没有 Nvidia GPU，或不需要运行 Ollama 和 vLLM 这类需要 GPU 的容器 AI 应用，可以跳过本文。
...
请先下载并准备好 NVIDIA 驱动与 CUDA 安装包（如 CUDA 12.8），并将安装包传输到目标机或执行 ocboot 的机器上可访问的路径。
...
9. 安装与配置 lxcfs
10. 配置主机设备映射（自动发现 NVIDIA 对应 `/dev/dri/renderD*` 并生成配置）
11. 验证安装结果
...
在运行 `setup-ai-env` 或带安装包参数的 `ai` 前，可先将安装包传到目标机，例如：
