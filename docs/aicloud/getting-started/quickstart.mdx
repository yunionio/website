...
- 关于 GPU
  ...
  - 如果没有 Nvidia GPU，部署后的环境就无法运行 ollama 这种推理容器实例，但可以运行 OpenClaw 和 dify 这种不依赖 GPU 的应用容器实例。<!-- 如果没有 Nvidia GPU，部署后的环境就无法运行 ollama 和 vllm 这种推理容器实例，但可以运行 OpenClaw 和 dify 这种不依赖 GPU 的应用容器实例。 -->
...
| OpencloudOS 8.x x86_64         | ✅   |
| OpencloudOS 9.x x86_64         | ✅   |
| AnolisOS 8.x x86_64            | ✅   |
...
若需在节点上安装或配置 NVIDIA 驱动与 CUDA 以运行 Ollama 等依赖 GPU 的 AI 容器应用，请参考：[配置 NVIDIA 与 CUDA 环境](./setup-nvidia-cuda)。<!-- 若需在节点上安装或配置 NVIDIA 驱动与 CUDA 以运行 Ollama、vLLM 等依赖 GPU 的 AI 容器应用，请参考：[配置 NVIDIA 与 CUDA 环境](./setup-nvidia-cuda)。 -->
