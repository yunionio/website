...
- 关于 GPU
  ...
  - 如果没有 Nvidia GPU，部署后的环境就无法运行 vLLM、SGLang 等推理容器实例，但可以运行 OpenClaw 和 dify 这种不依赖 GPU 的应用容器实例。
...
若需在节点上安装或配置 NVIDIA 驱动与 CUDA 以运行 vLLM、SGLang 等依赖 GPU 的 AI 容器应用，请参考：[配置 NVIDIA 与 CUDA 环境](./setup-nvidia-cuda)。
...
|---|---|---|---|---|
| OpenClaw | Agent | 开源自托管的个人智能体助手 | 不需要 | [OpenClaw 快速开始](../guides/llm-app/instance/openclaw#quickstart) |
| HermesAgent | Agent | Nous Research 开源自主 Agent | 不需要 | [HermesAgent 快速开始](../guides/llm-app/instance/hermes#quickstart) |
| Dify | Agent | LLM 应用开发与工作流编排平台（可对接推理服务） | 不需要 | [Dify 快速开始](../guides/llm-app/instance/dify#quickstart) |
| ComfyUI | Agent | 图像生成与节点式工作流应用 | 需要 | [ComfyUI 快速开始](../guides/llm-app/instance/comfyui#quickstart) |
| vLLM / SGLang 等 | AI推理 | 按模板类型部署的推理服务 | 需要 | [AI 推理快速开始](../guides/llm-inference/quickstart) |
