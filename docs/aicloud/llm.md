...
│                          LLM Service Layer                          │
├─────────────┬─────────────┬─────────────┬───────────────────────────┤
│  LLM 实例管理  │  即时模型服务  │  Dify 平台   │      MCP Agent           │
├─────────────┼─────────────┼─────────────┼───────────────────────────┤
│  Ollama/vLLM│  Model Cache │  Knowledge  │    Model Context          │
│  Container  │  Preload     │  Base       │    Protocol               │
└─────────────┴─────────────┴─────────────┴───────────────────────────┘
                              │
┌─────────────────────────────┼───────────────────────────────────────┐
│                    Compute Platform (KubeVirt/Container)           │
├─────────────────────────────┴───────────────────────────────────────┤
...
负责 LLM 服务实例的完整生命周期管理：

- **创建与部署**：基于 SKU 规格自动创建容器化 LLM 实例
- **模型管理**：支持从 Ollama 仓库拉取模型或挂载预置模型
- **弹性伸缩**：支持启动、停止、重启等操作
- **状态监控**：实时同步实例运行状态
...
- **模型缓存**：将模型文件缓存为镜像，实现秒级挂载
- **镜像管理**：支持模型镜像的导入、导出和分发
- **版本控制**：支持模型标签（Tag）管理
- **自动缓存**：支持自动将模型缓存到计算节点

#### 3. Dify 集成

集成 Dify.AI 开源 LLM 应用开发平台：

- **一键部署**：支持快速部署完整的 Dify 服务栈
- **知识库管理**：支持文档上传和向量化处理
- **工作流编排**：可视化 AI 工作流设计
- **多租户隔离**：支持多项目的资源隔离
...
实现 Model Context Protocol 的智能体服务：

- **工具调用**：支持调用云平台 API 作为工具
- **多轮对话**：支持带历史记录的上下文对话
- **流式响应**：支持 SSE 流式输出
- **双驱动支持**：支持 Ollama 和 OpenAI 驱动
