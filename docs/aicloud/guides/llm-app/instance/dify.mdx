...
4. **安装模型插件并配置模型供应商**：如需对接 OpenAI、OpenAI 兼容服务，或平台内的 vLLM 等下游推理服务，先在 Dify Web 后台的 **插件 → Marketplace** 安装对应插件，再配置模型供应商与下游推理服务。
...
- 对接平台内已就绪的 [推理部署](../../llm-inference/deployment)（如 vLLM / SGLang 类型）。

推荐操作流程：

1. 登录 Dify 控制台。
2. 进入 **插件 → Marketplace**。
3. 在 Marketplace 中搜索并安装对应插件，例如 `OpenAI`、`OpenAI-API-compatible` 或 `vLLM`。
...
- **vLLM 联动**：如果对接平台内的推理部署，建议先完成其可用性验证，再到 Dify 的 **插件 → Marketplace** 安装对应插件，最后回到模型供应商页做接入。
