...
3. **准备并挂载模型**：vLLM 实例需要挂载可加载的模型。建议先在 [推理模型库](./model-library) 中导入 Hugging Face 模型，再在模板或实例参数中选择对应模型版本。
...
vLLM 实例创建时需要有可挂载模型：可以由模板预挂载，也可以在实例创建参数中选择。按当前平台实现，vLLM 会从 `/data/models/huggingface` 下选择模型目录并启动服务；如果没有挂载模型，实例创建会被校验拦截，或者容器无法进入可服务状态。

如果模板或实例页面的模型相关字段里已经有目标模型可选，建议优先直接选择。这样实例创建完成后，平台会把对应模型挂载到 vLLM 的模型目录里，减少重复下载和版本漂移。

可参考以下流程：

1. 进入 **人工智能 → 模型库**。
2. 通过“导入社区模型”准备目标模型，模型类型选择 `vLLM`，来源通常选择 Hugging Face。
...
如果控制台里还没有可用的 vLLM 模板，可以先前往 **人工智能 → 推理 → 推理模板** 创建或编辑模板，再返回实例页面创建。vLLM 模板或实例必须具备 GPU 和挂载模型，否则无法正常创建或启动。

![](./images/instantce-create.png)

![](./images/vllm-instance-create.png)
