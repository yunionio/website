...
创建 vLLM 推理部署大致分为以下几步：

1. **准备 GPU 环境**：完成 [配置 NVIDIA 与 CUDA 环境](/docs/aicloud/getting-started/setup-nvidia-cuda)，并提前规划好 GPU 显存、内存与持久化存储空间。
2. **导入模型生成模板**：在 [推理模板](./template#import-modelscope) 通过 **导入模型**（从模型库 / Hugging Face / ModelScope / 本地路径）生成带挂载模型的 vLLM 模板；导入后可 **修改** 模板规格。
3. **创建推理部署**：在模板卡片点击 **部署**，或进入 **人工智能 → 推理 → 推理部署 → 新建** 并选择该模板，详见 [快速开始/创建](./quickstart#create-deployment)。
4. **chat / API 验证**：创建 [API Key](./quickstart#create-api-key) 后，在部署详情做 [chat 测试](./quickstart#chat-test)；可选在 [AI 网关接入](./quickstart#aiproxy-curl) 复制 curl。实例侧「连接信息」可作为直接访问引擎端口的补充手段。

下文补充 vLLM 在规格、模型目录与高级参数上的注意点；端到端操作与截图以 [快速开始](./quickstart) 为准。
### 1. 准备 GPU 环境

vLLM 依赖 GPU 做推理，请先完成 [配置 NVIDIA 与 CUDA 环境](/docs/aicloud/getting-started/setup-nvidia-cuda)，并重点确认以下几项：
...
推荐优先走 [导入模型](./template#import-modelscope) 生成模板。导入后如需调整规格：

1. 进入 **人工智能 → 推理 → 推理模板**，选择现有 vLLM 模板点击 **修改**。
2. 在规格配置中，选择合适的 CPU、内存、GPU 型号、GPU 数量和数据盘大小。
3. 在镜像字段中选择对应的 [推理镜像](./image)，保存模板。

为模板配置规格与镜像时，重点关注：

- **GPU 数量与显存**：直接决定模型能否加载，以及 `tensor-parallel-size` 的默认值。平台会按模板/SKU 中配置的 GPU 数量自动设置并行度，建议实例创建时保持 GPU 配置与模板一致。
...
vLLM 部署创建时需要有可挂载模型。**推荐**在推理模板页通过 [从 ModelScope 导入](./template#import-modelscope)（或 Hugging Face / 本地路径）一次性生成带挂载模型的模板。

也可先在 [模型文件](./model-library) 准备条目，再在模板中挂载。按当前平台实现，vLLM 会从 `/data/models/huggingface` 下选择模型目录并启动服务；如果没有挂载模型，部署创建会被校验拦截，或者容器无法进入可服务状态。
...
:::tip
推荐优先使用模型文件统一管理模型来源和版本。这样同一个模型可以被多个实例复用，也更方便排查“同名模型但版本不同”带来的差异。
:::

### 4. 创建推理部署

请按 [快速开始/创建](./quickstart#create-deployment) 操作：从模板卡片 **部署**，或 **推理部署 → 新建** 并选择已就绪的 vLLM 模板。模板须具备 GPU 与挂载模型。
...
1. 等待部署 **就绪**（见 [快速开始/创建](./quickstart#create-deployment)）。
2. 创建 [API Key](./quickstart#create-api-key)。
3. 在部署详情做 [chat 测试](./quickstart#chat-test)。
4. （可选）在 [AI 网关接入](./quickstart#aiproxy-curl) 复制 curl 调用示例。

![](./images/deployment-chat-test.png)

**补充：实例连接信息与直连引擎**

若需查看容器侧地址，可打开部署详情中的 **实例**，进入实例详情的 **连接信息**。vLLM 容器内默认监听 `8000`，对外地址以控制台为准。

![](./images/vllm-connect-info.png)

建议同时：

1. 确认实例状态为“运行中”。
2. 打开 **日志**，确认没有模型加载失败、显存不足等报错。
3. 如有需要，通过 **终端** 检查模型目录。
...
经 AI 网关访问时，请使用网关接入地址与 API Key，而不是仅依赖实例 IP。
...
- vLLM 服务日志：默认输出到容器标准输出，可在实例详情页 **日志** 中查看。
...
`customized_args` 用来补充 vLLM 启动参数。平台会在默认启动参数和模板/SKU 的后端参数基础上追加这些配置，并做一层安全限制。
...
其中，`tensor-parallel-size` 会根据模板/SKU 中配置的 GPU 数量自动设置。如果需要调整 `swap-space`、`max-model-len`、`gpu-memory-utilization` 等 vLLM 自身参数，应通过后端参数或 `customized_args` 显式传入。
...
| `--swap-space 8` | `swap-space` | `8` | 设置 vLLM 交换空间 |
...
如果你通过模型部署或批量副本能力创建 vLLM 服务，可以在部署层配置 GPU 显存利用率：

- `gpu_memory_utilization`：手动指定 0 到 1 之间的显存利用率，平台会转换为 vLLM 的 `--gpu-memory-utilization` 参数。
- `auto_gpu_memory_utilization`：由平台根据挂载模型大小、GPU 显存和 GPU 数量计算显存利用率，再转换为 `--gpu-memory-utilization` 参数。

> 截图占位：vLLM 模型部署中的 GPU 显存利用率配置

两者不能同时启用。自动计算依赖挂载模型和 GPU 设备信息；如果没有挂载模型，或者 GPU 型号缺少显存信息，平台会拒绝创建。自动计算结果会保留安全余量并限制在合理范围内，适合希望减少手工估算但仍由平台控制上限的场景。

如果你只是在普通推理部署上调参，也可以直接在 `customized_args` 中填写：
...
- **镜像选择**：参考 [推理镜像](./image)，使用明确版本（tag/digest）便于升级与回滚，并注意 CUDA、驱动与 vLLM 版本兼容性。

### 模型与版本

- **模型文件**：结合 [模型文件](./model-library) 统一管理模型来源、版本与复用策略。
...
- **升级与回滚**：通过 [推理镜像](./image) 控制运行时版本；变更前评估 CUDA、驱动、镜像与模型版本兼容性。
- **观测指标**：重点关注 GPU 利用率、显存占用、QPS、P95/P99 延迟、错误率。
- **日志排查**：模型加载失败、显存不足或高级参数错误时，可优先检查实例详情页的 **日志**；当前 vLLM 服务日志默认输出到容器标准输出。
...
![](./images/vllm-into-shell.png)
![](./images/vllm-shell-content.png)

### 怎么查看服务日志？

通过前端界面：打开对应的推理部署（再进入实例），进入详情页面，再点击 **日志**，就能查看 vLLM 的服务输出日志，方便用于错误排查。

![](./images/vllm-logs.png)
...
- 按 [配置 NVIDIA 与 CUDA 环境](/docs/aicloud/getting-started/setup-nvidia-cuda) 检查驱动、CUDA 版本与节点状态。
