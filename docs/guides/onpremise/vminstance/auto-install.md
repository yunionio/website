...
在您的 Kickstart 或 Autoinstall 配置文件中，**必须**包含以下状态标记来确保 CloudPods 能够正确跟踪安装进度：
...
- 如果配置文件中缺少 `KICKSTART_COMPLETED` 标记，系统将在超时后认为安装失败
...
### 配置文件编写建议
- **状态标记**：务必在配置文件中包含 `KICKSTART_COMPLETED` 标记
- **错误处理**：在关键步骤后添加错误检查和 `KICKSTART_FAILED` 标记
