...
host_local类型的IP子网配置在二层网络上，宿主机上host服务启动时，会拉取本宿主机所对接的二层网络的所有host_local IP子网，并将这些IP子网的网关配置在对应二层网络的网桥设备上。同时，配置openvswitch流表，实现：
（1）阻止容器网络内节点之间互相访问；
（2）实现容器访问外网的SNAT规则；
（3）配置端口映射规则，实现外部访问宿主机端口并转发到容器内端口的DNAT规则。
