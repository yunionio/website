...
每个服务二进制都运行在Kubernetes的Pod中，如何诊断Pod内服务组件的问题，请参考 [排查pod异常](./poderror)。
...
Kubernetes并不具备systemd停止和启动服务的控制能力，[暂停集群服务](./halt_cluster)介绍了通过operator暂停和恢复服务的方法。

使用老版本 ocboot 部署的 k3s 集群可能遇到内部证书失效的问题。[更新 k3s 集群证书](./k3s-renew-certs)介绍了使用 ocboot 脚本更新 control plane 和 worker 节点证书的方法。
