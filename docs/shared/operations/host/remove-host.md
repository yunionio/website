...
如果需要将一个计算节点（宿主机）下线，需要执行以下步骤确保一个计算节点干净下线。

1. 将该宿主机上的虚拟机/容器全部迁移走。确保该宿主机上没有虚拟机/容器，该宿主机上的本地存储没有磁盘
...
注意：如果删除宿主机记录失败，可能的原因是宿主机上还有未清理的虚拟机/容器，或者宿主机的本地存储有未清理的磁盘。发生这个情况时，可以通过 climc 容器的 `clean_host.sh` 脚本自动清理该宿主机上残留的虚拟机/容器和本地磁盘，并且删除宿主机的数据库记录
