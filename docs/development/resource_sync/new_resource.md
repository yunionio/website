...
创建 pkg/compute/tasks/elastic_search_delete_task.go 文件, 编写删除任务。
...
编辑 cmd/climc/shell/compute/elastic_search.go 文件，添加 climc 删除命令。
...
此步骤仅适用于新资源接入，已有资源不涉及此操作，编辑 pkg/compute/models/purge.go 文件，追加以下内容。
