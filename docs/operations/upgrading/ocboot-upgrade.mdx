...
升级的版本号可以到 [CHANGELOG 3.10 页面](../../development/changelog/release-3.10/) 查询。
...
降级的原理是修改各个服务的 image version，比如现在的版本是 v3.10.7，然后想要降级到 v3.10.6。

登录到第一个控制节点执行如下命令，会降级服务到 v3.10.6 。

```bash
$ /opt/yunion/bin/ocadm cluster update --version v3.10.6 --wait
```

降级会重新拉取新的镜像，可以再开一个窗口，登录第一个控制节点，使用下面的命令查看每个 pod 的更新情况。
